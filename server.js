...
// 조별 시간 설정
const teamSchedules = {
    'a-team': { hour: 7, minute: 5, timeFilter: '07:00:00' },  // 07:05에 실행, 07:00 이전 데이터
    'b-team': { hour: 15, minute: 5, timeFilter: '15:00:00' }, // 15:05에 실행, 15:00 이전 데이터
    'c-team': { hour: 23, minute: 5, timeFilter: '23:00:00' }  // 23:05에 실행, 23:00 이전 데이터
};
...
async function fetchAutobeDataForTeam(team) {
    ...
    try {
        ...
        console.log(`${team} Autobe 쿼리 실행 시작... (시간 필터: ${schedule.timeFilter} 이전)`);
        ...
        const result = await connection.execute(`
            ...
            b AS (
                ...
                FROM (
                    ...
                    AND TO_CHAR(start_time,'HH24:MI:SS') < :timeFilter
                ) b_sub
                WHERE rn = 1
            )
            SELECT
        ...
        `, { timeFilter: schedule.timeFilter });
...
function setupSchedulers() {
    ...
    const schedulerInterval = setInterval(() => {
        ...
        // A조: 매일 07:05
        if (currentTime === '07:05' && lastExecuted['a-team'] !== currentDate) {
            console.log('A조 스케줄 실행 (07:05)');
            updateTeamData('a-team').catch(err => console.error('A조 업데이트 실패:', err));
            lastExecuted['a-team'] = currentDate;
        }
        
        // B조: 매일 15:05
        if (currentTime === '15:05' && lastExecuted['b-team'] !== currentDate) {
            console.log('B조 스케줄 실행 (15:05)');
            updateTeamData('b-team').catch(err => console.error('B조 업데이트 실패:', err));
            lastExecuted['b-team'] = currentDate;
        }
        
        // C조: 매일 23:05
        if (currentTime === '23:05' && lastExecuted['c-team'] !== currentDate) {
            console.log('C조 스케줄 실행 (23:05)');
